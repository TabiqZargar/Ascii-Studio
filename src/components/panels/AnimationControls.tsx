...
export default function AnimationControls() {
  ...
  const cached = animation.cachedCount;
  const allCached = cached >= total;
  const pendingCount = animation.pendingFrames.length;
  ...
  return (
    <div className="flex flex-col gap-2 border-t border-zinc-800/50 bg-zinc-900/80 px-4 py-2 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2">
          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-zinc-800">
            <div
              ...
              style={{ width: `${total > 0 ? (cached / total) * 100 : 0}%` }}
            />
          </div>
          <span className="text-[10px] text-zinc-500 tabular-nums">
            {cached}/{total} frames
            {pendingCount > 0 && <span className="ml-1 text-zinc-600">({pendingCount} queued)</span>}
          </span>
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() => dispatch({ type: "SET_CURRENT_FRAME", index: animation.currentFrame + 1 })}
          disabled={animation.currentFrame >= total - 1}
          className="rounded-md bg-zinc-800/50 px-2 py-1 text-[10px] text-zinc-400 hover:bg-zinc-700/50 hover:text-zinc-200 transition-colors disabled:opacity-30"
          title="Next frame"
        >
          ▶
        ...
        <span className="text-[10px] text-zinc-500 tabular-nums">
          {animation.currentFrame + 1}/{total}
        </span>

        <span className="text-[10px] text-zinc-600 tabular-nums">
          {animation.sourceFps}→{animation.fps}fps
        </span>
